          ...
          {orders.slice(0, 10).map((order) => (
            <div key={order._id} className="p-4">
              ...
              <div className="text-sm text-gray-600 mb-2">
                <p><strong>Máquina:</strong> {order.selectedMachine} ({order.machineType})</p>
                <p><strong>Quantidade:</strong> {order.quantity}</p>
                <p><strong>Pagamento:</strong> {order.paymentMethod === "avista" ? "À vista" : "Parcelado"}</p>
                <p><strong>Total:</strong> R$ {order.totalPrice.toFixed(2)}</p>
                {order.installmentPrice && (
                  <p><strong>Parcela:</strong> 12x R$ {order.installmentPrice.toFixed(2)}</p>
                )}
              </div>
